  ...
  return (
    ...
    >
      <section className="flex flex-col bg-cover bg-center bg-no-repeat bg-win bg-indigo-300  rounded-xl shadow-3d text-center transform -translate-x-1/2 -translate-y-1/2 absolute top-1/2 left-1/2">
        ...
        <div className="flex flex-col  relative h-[4rem] item-center justify-center bg-indigo-400 rounded-tl-xl rounded-tr-xl">
       ...
       <div className="flex flex-col mt-3">
        ...
        <h3 className="text-pink-500 mt-[-1rem] px-2">
            {mainPrompt}
        </h3>
       </div>
      
        {/* The button and its function on the window */}
        <div className="flex item-center justify-center py-[1rem] ">
        <button
          onClick={pageJump}
          className="flex item-center justify-center bg-game-pink border-white w-[7rem] text-white font-bold py-2 px-4 rounded-lg"
        >
          {buttonPrompt}
        </button>
        </div>
